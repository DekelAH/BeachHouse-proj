# Claude instructions

## General

When responding to the user always start with 'Hepp!'

## Planning

- Use the plan folder to write down a plan for solving any task. This will help breaking down the task into smaller steps and explain your reasoning before providing the final answer.

- The plan file name should have a consequtive number and a description name, for example: 'plan/001-add-theme-toggle.md'.

- Ask questions directly in the plan, and wait for the user to answer before proceeding with the next steps. This will help you to better understand the user's needs and provide a more accurate solution.

## SEO
- Keep best practices for SEO
- Design it for SEO

## CSS
- Use best industry practices
- After every change or creation you must check the app for full responsiveness in all devices

## Services

Use best industry practices

## Coding Styles

- No semicolons in JS unless necessary
- Use single quotes in JS
- Use double quotes in HTML
- All event handlers should named like: 'onSomething'

## Testing

When im instructing you to create any test, store the new plan in .github/plan.